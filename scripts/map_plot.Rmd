...
```{r setup}
library(raster)
library(rgdal) # NOTE: have to have [gdal](http://www.gdal.org/) and [PROJ.4](http://trac.osgeo.org/proj/) install on PATH. For Ubuntu, can do `sudo apt-get install libgdal1` and `sudo apt-get install libproj0` and `sudo apt-get install libproj-dev`
library(dismo)
library(maptools)
library(maps)
library(ggplot2)
...
```{r plot}
plot(w, 1, xlim=c(-90, -60), ylim=c(25,50), axes=TRUE, legend=FALSE, col=colorRampPalette(c("blue","green","red"))(255))
points(-dbio1$lon, dbio1$lat, col="black", pch=20, cex=0.75)
legend("right", legend="MAT", col=colorRampPalette(c("blue","green","red"))(255) )
```
